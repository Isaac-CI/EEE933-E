...
```
## 1. Resumo
O presente estudo avalia o impacto de duas configurações distintas (Alg1 e Alg2) do algoritmo de Evolução Diferencial (DE) no desempenho da otimização de 149 instâncias da função de Rosenbrock. A metodologia empregou um planejamento experimental com blocagem aleatória (64 blocos) e Análise de Variância (ANOVA) para comparar os algoritmos, controlando a variabilidade introduzida pelas diferentes dimensões das instâncias. Os resultados da ANOVA (R2=0.984) indicaram que a blocagem foi altamente eficaz (Eficiência ≈ 28) e que existe uma diferença estatisticamente significativa no desempenho das duas configurações (p<2e−16). As premissas do modelo (normalidade dos resíduos e homocedasticidade) foram validadas. 

Uma análise pareada subsequente, utilizando o teste não paramétrico de Wilcoxon (devido à não normalidade das diferenças intra-bloco), confirmou a diferença significativa (p≈3.61×10−12). O cálculo do tamanho do efeito (Cohen’s d=2.55) revelou que a magnitude da diferença é extremamente grande. Conclui-se que o Algoritmo 1 apresenta um desempenho consistentemente superior ao Algoritmo 2, sendo a configuração recomendada para esta classe de problemas.
...
### 3.1 Formulação das Hipóteses e Planejamento Experimental
...
### 3.3 Instâncias de Teste e Configurações dos Modelos
...
### 3.4 Geração e Formatação dos Dados
...
# Carrega os dados que foram gerados e salvos anteriormente
data <- read.csv("resultados_rosenbrock_medias.csv")

# Ordenar colunas
data$Algorithm <- factor(data$Algorithm, levels = c("Alg1", "Alg2"))
data$Block     <- factor(data$Block, levels = as.character(1:64))
```
...
```{r fig-eda, echo=TRUE, fig.width=13, fig.height=5, out.width="100%", fig.align='center', fig.cap='Visualização de desempenho entre os algoritmos.'}
library(ggplot2)
...
p <- ggplot(aggdata, aes(x = Block, 
  ...
  geom_point(size = 2) +
  labs(x = "Blocos", y = "Desempenho (Y)", colour = "Algoritmo") +
...
### 4.3 Validação das Premissas do Modelo

O teste ANOVA é paramétrico, portanto, é necessário verificar algumas premissas fundamentais para garantir a validade dos resultados. As principais premissas são anormalidade dos resíduose a homocedasticidade (variância constante dos erros).

#### 4.3.1 Normalidade dos Resíduos:
...
No caso em análise, o teste de *Shapiro–Wilk* apresentou W = 0.99622 e *p*-value = 0.9841, o que é muito superior a 0,05. Assim, não se rejeita a hipótese nula, concluindo-se que os resíduos seguem uma distribuição aproximadamente normal.
...
```{r fig-diagnostics, echo=TRUE, fig.width=10, fig.height=8, out.width="100%", fig.align='center', fig.cap='Testes gráficos para ver normalidade e independência.'}
# Testes gráficos para ver normalidade e independência
par(mfrow = c(2, 2))  # divide a área em 4 gráficos
...
#### 4.3.2 Homogeneidade de Variâncias (Homocedasticidade):
...
Para verificar a homocedasticidade de forma quantitativa, pode-se realizar o teste de *Fligner-Killeen*. O *p*-valor encontrado foi de 0.9146, muito maior que 0.05, não sendo capaz, portanto, de rejeitar a hipótese de homocedasticidade.

```{r, echo=TRUE, eval=TRUE}
group <- aggdata$Algorithm
fligner.test(res ~ group)
```

### 4.4 Tamanho do Efeito
...
```{r fig-diffplots, echo=TRUE, fig.width=12, fig.height=4, out.width="100%", fig.align='center', fig.cap='Visualização das diferenças de desempenho intra-blocos.'}
# Visualização das diferenças de desempenho intra-blocos
par(mfrow = c(1, 3))  # 1 linha, 3 colunas
hist(wide$diff, breaks = 15,
     main = "Histograma das diferenças",
     xlab = "Alg2 - Alg1", col = "lightgray", border = "white")

qqnorm(wide$diff, main = "Gráfico Q-Q das diferenças")
qqline(wide$diff, col = "red", lwd = 2)

boxplot(wide$diff,
        main = "Boxplot das diferenças",
        ylab = "Alg2 - Alg1", col = "lightgray", border = "gray30")

par(mfrow = c(1, 1))  # restaura layout
```
...
#### 4.4.2 Cálculo do Tamanho do Efeito:
